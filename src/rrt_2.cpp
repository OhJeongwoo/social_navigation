...
class RRT{
    ...
    int kernel_half_;
    int kernel_size_;
    int rrt_kernel_half_;
    int rrt_kernel_size_;
    double rrt_alpha_;
    vector<vector<int>> rrt_kernel_;
    ...
    RRT(){
        ...
        rrt_cost_map_ = Mat(img_w_, img_h_, CV_8UC1, Scalar(0));
        ...
        rrt_cost_ = 203;
        rrt_kernel_half_ = 30;
        rrt_kernel_size_ = rrt_kernel_half_ * 2 + 1;
        rrt_alpha_ = 0.003;
        build_kernel();
        build_rrt_kernel();
    ...
    void build_rrt_kernel(){
        for(int i = -rrt_kernel_half_; i <= rrt_kernel_half_; i++){
            vector<int> kernel_row;
            for(int j = -rrt_kernel_half_; j <= rrt_kernel_half_; j++) kernel_row.push_back(int(rrt_cost_ * exp(-rrt_alpha_ * (i * i + j * j))));
            rrt_kernel_.push_back(kernel_row);
            for(int j = 0;j<rrt_kernel_size_;j++) cout << kernel_row[j] << " ";
            cout << endl;
        }
    }
    ...
    void callback(const social_navigation::RRT::ConstPtr& msg){
        ...
        vector<point> path;
        if(msg->K==0) path = rrt(msg->option);
        else{
            vector<vector<point>> paths;
            for(int k_=0;k_<msg->K;k_++){
                path = rrt(true);
                for(point pt : path){
                    pixel p = get_pixel(pt);
                    for(int i = -rrt_kernel_half_; i <= rrt_kernel_half_; i++){
                        for(int j = -rrt_kernel_half_; j <= rrt_kernel_half_; j++){
                            int px = p.first + i;
                            int py = p.second + j;
                            // rrt_cost_map_.at<uchar>(px,py) = max<uchar>(rrt_cost_map_.at<uchar>(px,py), kernel_[i+kernel_half_][j+kernel_half_] * max_cost_);
                            // rrt_cost_map_.at<uchar>(px,py) = max<uchar>(rrt_cost_map_.at<uchar>(px,py), rrt_kernel_[i+rrt_kernel_half_][j+rrt_kernel_half_]);
                        }
                    }
                }
    ...
    double get_cost(point p){
        ...
        double rt = double(cost_map_.at<uchar>(px, py));
        rt = max(rt, double(local_cost_map_.at<uchar>(px, py)));
        rt = max(rt, double(rrt_cost_map_.at<uchar>(px, py)));
        return rt / 255.0;
    ...
    vector<point> rrt(bool option){
        ...
        x_min_ = min<double>(goal.first, root.first) - 10.0;
        x_max_ = max<double>(goal.first, root.first) + 10.0;
        y_min_ = min<double>(goal.second, root.second) - 10.0;
        y_max_ = max<double>(goal.second, root.second) + 10.0;
    ...
    void draw2(const vector<vector<point>>& paths, point goal){
        local_map_ = imread(costmap_file_, CV_8UC1);
        string local_map_save_path = pkg_path_.str() + "test2.png";
        
        for(int i = 0; i < img_w_; i++){
            for(int j =0; j < img_h_; j++) {
                local_map_.at<uchar>(i, j) = max(local_map_.at<uchar>(i, j), local_cost_map_.at<uchar>(i, j));
                local_map_.at<uchar>(i, j) = max(local_map_.at<uchar>(i, j), rrt_cost_map_.at<uchar>(i, j));
            }
        }
        ...
        for(const vector<point>& path : paths){
            ...
            for(int i =0;i<sz-1;i++){
                pixel cur = get_pixel(path[i]);
                pixel nxt = get_pixel(path[i+1]);
                cv::line(local_map_, Point(cur.second, cur.first), Point(nxt.second, nxt.first), Scalar(255), 2);
            }
        }
