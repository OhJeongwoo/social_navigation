...
from utils.preprocess_png import get_segmentation_from_png
...
class YNet:
	def __init__(self, obs_len, pred_len, params, image_path=None):
		"""
		...
		self.test_images = get_segmentation_from_png(image_path, params, self.division_factor)
	...
	def predict(self, trajectory, params, num_goals=20, num_traj=1, device=None):
		if device is None:
			device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
		...
		# print('Preprocess data')
		# test_images = get_segmentation_from_png(image_path, params, self.division_factor)
